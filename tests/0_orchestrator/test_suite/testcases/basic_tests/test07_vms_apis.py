...
class TestVmsAPI(TestcasesBase):
    ...
    def setUpClass(cls):
        ...
        blockcluster_data, objectcluster_data = self.get_clusters_data(nodes)

        storageclusters = self.storageclusters_api.get_storageclusters()
        storagecluster = self.get_available_cluster_type(storageclusters.json(), "block")
        if not storagecluster:
