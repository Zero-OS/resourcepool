...
class TestVmsAPI(TestcasesBase):
    ...
    def test013_import_export_vm(self):
        """ GAT-158
        **Test Scenario:**

        #. Get random nodid (N0).
        #. Create virtual machine (VM0) on node (N0).
        #. Stop virtual machine(VM0) ,should succeed.
        #. Export virtual machine (VM0) on ftpserver, should succeed .
        #. Import virtual machine (VM0) from ftpserver as (VM1), should succeed.
        #. Check that (VM1) has same specs as (VM0), should succeed.
        #. Execute command on virtual machine (VM1), should succeed.

        """
        self.lg.info(" Stop virtual machine(VM0) ,should succeed.")
        response = self.vms_api.post_nodes_vms_vmid_stop(nodeid=self.nodeid, vmid=self.data['id'])
        self.assertEqual(response.status_code, 204, response.content)
        ...
        self.lg.info(" Export virtual machine (VM0) on ftpserver, should succeed .")
        response, data = self.vms_api.post_nodes_vms_vmid_export(nodeid=self.nodeid, vmid=self.data['id'])
        self.assertEqual(response.status_code, 200, response.content)
        self.lg.info("vm exported in ftp server %s"%data["url"])
        exported_vm_url = response.json()["url"]

        self.lg.info("Import virtual machine (VM0) from ftpserver as (VM1), should succeed.")
        response, new_vm = self.vms_api.post_nodes_vms_import(nodeid=self.nodeid,
                                                              vdiskstorage=TestVmsAPI.vdiskstorageid,
                                                              url=exported_vm_url)
        self.assertEqual(response.status_code, 201, response.content)

        self.lg.info("Check (VM1) has same specs as (VM0), should succeed")
        response = self.vms_api.get_nodes_vms_vmid(self.nodeid, self.data['id'])
        self.assertEqual(response.status_code, 200)
        original_vm_data = response.json()

        response = self.vms_api.get_nodes_vms_vmid(self.nodeid, new_vm['id'])
        self.assertEqual(response.status_code, 200)
        new_vm_data = response.json()

        keys_to_check = ['memory', 'cpu', 'nics']
        for key in keys_to_check:
            self.assertEqual(original_vm_data[key], new_vm_data[key])

        self.lg.info('[*] Execute command on virtual machine (VM1), should succeed')
        vm1_ip_address = self.get_vm_default_ipaddress(new_vm['id'])
        response = self.execute_command_inside_vm(self.ssh_client, vm1_ip_address, 'uname')
        self.assertEqual(response.state, 'SUCCESS')
        self.assertEqual(response.stdout.strip(), 'Linux')
