...
class TestVmsAPI(TestcasesBase):
    ...
    def setUpClass(cls):
        ...
        clusters_data = self.get_clusters_data(nodes)
        blockcluster_data = clusters_data["blockcluster"]
        objectcluster_data = clusters_data["objectcluster"]
        storageclusters = self.storageclusters_api.get_storageclusters()
        blockcluster = self.get_available_cluster_type(storageclusters.json(), "block")
        if not blockcluster:

            if not blockcluster_data["servers"]:
                self.skipTest('[*] No free disks to create storage cluster')

            self.lg.info('[*] Create blockcluster')
            response, cluster = self.storageclusters_api.post_storageclusters(nodes=nodes, driveType=blockcluster_data["type"], servers=blockcluster_data["servers"] )
            self.assertEqual(response.status_code, 201)

            self.lg.info('[*] blockcluster is created with specs : {}'.format(cluster))
            blockcluster = cluster['label']
        ...
        if not objectcluster:
            if len(objectcluster_data["nodes"]) >1:
                ...
                self.lg.info('[*] objectcluster is created with specs : {}'.format(cluster))
        ...
        response, vdiskstorage = self.vdisks_api.post_vdiskstorage(storagecluster=blockcluster, objectCluster=objectcluster)
        self.assertEqual(response.status_code, 201)
        ...
        TestVmsAPI.objectcluster = objectcluster
        TestVmsAPI.blockcluster = blockcluster
    ...
    def tearDownClass(cls):
        ...
        self.vdisks_api.delete_vdiskstorage(TestVmsAPI.vdiskstorageid)
