...
def install(job):
    ...
    if service.model.data.templateVdisk:
        ...
        clusterardb = "{ip}:{port}".format(**cluster.get_config()['metadataStorage'][0]['master'])
        ...
        try:
            if template.scheme in ('', 'ardb'):
                ...
                CMD = '/bin/copyvdisk -sourcetype direct -targettype direct {src_name} {dst_name} {masterardb} {clusterardb}'
