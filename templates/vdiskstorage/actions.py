...
def input(job):
    service = job.service
    args = job.model.args
    storagecluster.block_service = service.aysrepo.serviceGet(instance=args.get('blockCluster'), role='storage_cluster')
    if args.get('objectCluster'):
        if args.get('slaveCluster'):
            ...
            if storagecluster.block_service.model.data.nrServer < slave_cluster_service.model.data.nrServer:
