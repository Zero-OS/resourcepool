...
In order for the Orchestrator to know which flists and version of JumpScale to use, and which Zero-OS version is required on the nodes, create the following blueprint in `<j.dirs.VARDIR>/cockpit_repos/orchestrator-server/blueprints/configuration.bp`:
...
```bash
cd <j.dirs.VARDIR>/cockpit_repos/orchestrator-server
...
Create a new blueprint `<j.dirs.VARDIR>/cockpit_repos/orchestrator-server/blueprints/network.bp` and depending on the available 10GE+ network infrastructure specify following configuration:
...
So edit `<j.dirs.VARDIR>/cockpit_repos/orchestrator-server/blueprints/bootstrap.bp` as follows:
```yaml
